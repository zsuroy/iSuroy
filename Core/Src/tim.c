 ...
 * @brief 定时器中断回调函数
 * @note
 * HAL_TIM_Base_Start_IT(&htim2); //while(1)前，开启定时器中断: 必须要先开启才能进入中断处理回调函数
 * @param htim 
 */
 ...
 * @brief 占空比设置
 * @note
 * 若效果不明显可以直接将其放入 while(1){} 主程序中，且仅保留此段内容调试
 * PWM_Value 已传入
 * @return uint32_t 
 */
uint32_t TIM_PWM_Set(void)
{
  __HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_3, PWM_Value); // 设置占空比函数
  PWM_Value++; //占空比值加1
  if(PWM_Value>=499)PWM_Value=0; // 占空比达到最大值后清0，499为调试设置的TIM3最大值
  HAL_Delay(10); //延时10ms
  return PWM_Value;
}
